...
@Extension
public class RunImageBuildStepDescriptor extends BuildStepDescriptor<Builder> {
    ...
    public RunImageBuildStepDescriptor() {
        super(RunImageBuildStep.class);
        load();
    }
    ...
    public FormValidation doCheckFallbackTag(@QueryParameter(fixEmpty = true) String value) {
        if(value != null && containsVariables(value)) {
            return FormValidation.warning("Unable to validate image tag as it contains a variable");
        }
        if(isValid(value, ID_PATTERN)) {
            return FormValidation.ok();
